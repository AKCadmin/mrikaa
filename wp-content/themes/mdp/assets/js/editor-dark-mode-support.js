/* global mdpIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'mdp-supports-dark-theme' ) ) {
	mdpDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function mdpDarkModeEditorInit( attempt ) {
	...
	if ( mdpIsDarkMode() ) {
		if ( null === container ) {
			...
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						mdpDarkModeEditorInit( attempt + 1 );
					},
